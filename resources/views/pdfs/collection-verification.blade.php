...
<body>
    ...
    <div class="section">
        <h3>Commodity Allocation & Collection Details</h3>
        <table class="table">
            <thead>
                <tr>
                    <th>Commodity</th>
            ...
            <tbody>
                @foreach($commodity_allocations as $allocation)
                    <tr>
                        ...
                        <td>{{ number_format($allocation->allocated_quantity, 2) }}</td>
                        <td>{{ number_format($allocation->collected_quantity ?? 0, 2) }}</td>
                        <td>
                            @php
                                $shortage = $allocation->allocated_quantity - ($allocation->collected_quantity ?? 0);
                                $balance = ($allocation->collected_quantity ?? 0) - $allocation->allocated_quantity;
                            @endphp
                            @if($shortage > 0)
                                <span style="color: #dc2626;">{{ number_format($shortage, 2) }} Short</span>
                            @elseif($balance > 0)
                                <span style="color: #059669;">{{ number_format($balance, 2) }} Excess</span>
                            @else
                                <span>Balanced</span>
                            @endif
                        </td>
        ...
        @if($verification->collection_notes)
    ...
    <div class="photo-section">
        <div class="photo-container">
            <div class="photo-label">Verification Photo</div>
            @if($verification->commodity_photo && Storage::exists($verification->commodity_photo))
                <img src="{{ Storage::url($verification->commodity_photo) }}" alt="Collection verification photo" class="photo">
