...
@section('content')
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8">
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="text-3xl font-bold text-gray-900">Return Verification</h1>
                ...
                <div class="xl:col-span-1 space-y-6">
                            ...
                            <div class="border-t pt-4">
                                <dl class="space-y-3">
                                    ...
                                    <div class="flex justify-between">
                                        ...
                                        <dd class="text-sm text-indigo-600 capitalize">{{ $application->season->loan_type ?? 'Complete Loan' }}</dd>
                                    </div>
                                </dl>
                            </div>
                        </div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                        <div class="flex items-center mb-4">
                            ...
                            <h3 class="text-lg font-semibold text-gray-900">Collection Verified</h3>
                        </div>
                        <p class="text-sm text-gray-600 mb-3">
                            Collection was verified on {{ $application->collection_verification?->created_at?->format('M j, Y \a\t g:i A') ?? 'Date not available' }}
                        </p>
                        <div class="text-sm text-gray-600">
                            <span class="font-medium">Agent:</span> {{ $application->collection_verification?->agent?->user?->name ?? 'N/A' }}
                        </div>
                    </div>
                </div>

                <!-- Return Details Section (Center) -->
                <div class="xl:col-span-2 space-y-6">
                    <!-- Return Commodity Details -->
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                        <h3 class="text-xl font-semibold text-gray-900 mb-6">Return Commodity Details</h3>
                        ...
                        <form id="returnForm" enctype="multipart/form-data">
                            ...
                            <!-- Location Data -->
                            <input type="hidden" name="location_lat" id="location_lat" readonly />
                            <input type="hidden" name="location_lng" id="location_lng" readonly />

                            <!-- Expected Return Info -->
                            @if($expected && $expected['quantity'])
                            <div class="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                                <div class="flex items-center mb-2">
                                    <svg class="w-5 h-5 text-blue-600 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                    </svg>
                                    <span class="text-sm font-medium text-blue-800">Expected Return Information</span>
                                </div>
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                    <div>
                                        <span class="text-gray-600">Expected Quantity:</span>
                                        <span class="font-semibold text-blue-900 ml-2">{{ number_format($expected['quantity'], 2) }} {{ $expected['unit'] ?? '' }}</span>
                                    </div>
                                    <div>
                                        <span class="text-gray-600">Commodity:</span>
                                        <span class="font-semibold text-blue-900 ml-2">{{ $expected['commodity'] ?? 'N/A' }}</span>
                                    </div>
                                </div>
                                @if($expected['price_used'])
                                <div class="mt-2 text-sm">
                                    <span class="text-gray-600">Price Used:</span>
                                    <span class="font-semibold text-blue-900 ml-2">₦{{ number_format($expected['price_used'], 2) }} per {{ $expected['unit'] ?? 'unit' }}</span>
                                </div>
                                @endif
                            </div>
                            @endif
                            ...
                            <div class="mb-6">
                                <label for="returned_quantity" class="block text-lg font-semibold text-gray-900 mb-4">
                                    Returned Quantity *
                                </label>
                                <div class="relative max-w-xs">
                                    <input type="number" id="returned_quantity" name="returned_quantity"
                                           ...
                                           class="w-full px-4 py-3 text-lg border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                                    <div class="text-sm text-gray-600 mt-2">
                                        Quantity of {{ $expected['commodity'] ?? 'commodity' }} returned
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="border-t pt-6">
                                <label class="block text-lg font-semibold text-gray-900 mb-4">Return Photo *</label>
                                <div class="border-2 border-dashed border-gray-300 rounded-xl p-8 text-center hover:border-indigo-500 transition-colors">
                                    <input type="file" name="photo" id="returnPhoto" accept="image/*" required hidden>
                                    <div class="cursor-pointer" onclick="document.getElementById('returnPhoto').click()">
                                        ...
                                        <p class="text-gray-600 font-medium">Click to upload return photo</p>
                                        <p class="text-sm text-gray-500">PNG, JPG up to 4MB</p>
                                    </div>
                                    <img id="returnPhotoPreview" class="mt-4 w-full h-64 object-cover rounded-lg hidden border">
                                </div>
                                <button type="button" id="captureReturnPhoto" class="mt-3 inline-flex items-center px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">
                                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.828-1.472A2 2 0 0110.153 4h3.694a2 2 0 011.664.89l.828 1.472A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z"></path>
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 13a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                    </svg>
                            ...
                            <!-- Shortfall Reason -->
                            <div class="mt-6">
                                <label for="shortfall_reason" class="block text-lg font-semibold text-gray-900 mb-4">
                                    Shortfall Reason (if applicable)
                                </label>
                                <textarea id="shortfall_reason" name="shortfall_reason" rows="4"
                                          placeholder="If returned quantity is less than expected, explain why..."
                                ...
                                <p class="text-sm text-gray-500 mt-1">Only required if return quantity is less than expected</p>
                            </div>
                            ...
                            <div class="mt-8 border-t pt-8">
                                ...
                                <div class="mb-6">
                                    ...
                                    <div class="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center hover:border-indigo-500">
            ...
            <div class="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
                ...
                <div class="mb-6">
                    <p class="text-gray-600 text-sm leading-relaxed">
                        Please allow location access to verify your return activity. Your location is securely recorded with the verification for audit purposes and to prevent fraudulent return attempts.
                    </p>
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const returnPhotoInput = document.getElementById('returnPhoto');
            ...
            function getUserLocation() {
                ...
                if (shouldUseMockCoordinates()) {
                    ...
                    document.getElementById('location_lat').value = 10.286;
                    document.getElementById('location_lng').value = 11.167;
                ...
                if (navigator.geolocation) {
                    navigator.geolocation.getCurrentPosition(function(position) {
                        ...
                        document.getElementById('location_lng').value = longitude;
                        locationAcquired = true;
                        ...
                        switch(error.code) {
                            case error.PERMISSION_DENIED:
                                modalMessage = 'Please allow location access in your browser settings to verify your return activity.';
                                break;
            ...
            returnPhotoInput.addEventListener('change', function(e) {
                ...
                if (file) {
                    ...
                    reader.onload = e => {
                        returnPhotoPreview.src = e.target.result;
            ...
            document.getElementById('captureReturnPhoto').addEventListener('click', function() {
                    ...
                    .then(function(stream) {
                        ...
                        modal.innerHTML = `
                            <div class="bg-white rounded-lg p-6 max-w-lg w-full mx-4">
                                ...
                                <div class="flex justify-center mt-4">
                                    <button id="captureBtn" class="px-6 py-3 bg-indigo-600 text-white rounded-lg">Capture</button>
                                </div>
                            </div>
                        `;
                        ...
                        document.getElementById('captureBtn').addEventListener('click', function() {
                            ...
                            tempCanvas.toBlob(function(blob) {
                                ...
                                dt.items.add(file);
                                returnPhotoInput.files = dt.files;
            ...
            form.addEventListener('submit', function(e) {
                ...
                .then(data => {
                    if (data.message) {
                        window.location.href = '{{ route('agent.verify.return') }}';
                    } else {
            ...
            form.addEventListener('submit', function(e) {
                ...
                if (!locationAcquired) {
                    ...
                    const modalContent = locationModal.querySelector('.bg-white');
                    const messageDiv = modalContent.querySelector('p');
                    messageDiv.textContent = 'Location permission is required to complete the return verification. Please allow location access.';
                    allowLocationText.textContent = 'Allow Location';
                    return;
                }
