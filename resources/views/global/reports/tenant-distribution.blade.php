            ...
            <nav class="flex mb-4" aria-label="Breadcrumb">
                <ol class="inline-flex items-center space-x-1 md:space-x-3">
                    <li class="inline-flex items-center">
                        <a href="{{ route('global.reports.index') }}" class="text-gray-700 hover:text-blue-600 dark:text-gray-300">
            ...
            <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                <div>
                    ...
                    <p class="text-gray-600 dark:text-gray-400 mt-2">
                        <strong>{{ $season->name }}</strong> - Distribution details for <strong>{{ ucfirst($tenant->id) }}</strong>
                    </p>
                </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg">
                <div class="flex items-center justify-between">
                    <div>
                        <h3 class="text-2xl font-bold text-gray-900 dark:text-white">{{ $reportData['summary']['total_farmers'] }}</h3>
            ...
            <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg">
                <div class="flex items-center justify-between">
                    <div>
                        <h3 class="text-2xl font-bold text-gray-900 dark:text-white">{{ $reportData['summary']['farmers_collected'] }}</h3>
            ...
            <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg">
                <div class="flex items-center justify-between">
                    <div>
                        <h3 class="text-2xl font-bold text-gray-900 dark:text-white">{{ $reportData['summary']['farmers_returned'] ?? 0 }}</h3>
            ...
            <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg">
                <div class="flex items-center justify-between">
                    <div>
                        <h3 class="text-2xl font-bold text-gray-900 dark:text-white">{{ count($reportData['commodity_distribution']) }}</h3>
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right">
                                ...
                                <div class="text-xs {{ $commodity['collection_variance'] > 0 ? 'text-orange-500' : 'text-red-500' }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right">
                                ...
                                <div class="text-xs text-gray-500 dark:text-gray-400">
                                    Expected: {{ number_format($commodity['return_metrics']['expected_return'], 2) }}
                                </div>
                                @if($commodity['return_metrics']['return_variance'] != 0)
                                <div class="text-xs {{ $commodity['return_metrics']['return_variance'] > 0 ? 'text-red-500' : 'text-green-500' }}">
                                    Variance: {{ number_format(abs($commodity['return_metrics']['return_variance']), 2) }}
                                </div>
                                @endif
                                @else
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                @php
                                    $collectionRate = $commodity['distributed_quantity'] > 0
                                        ? ($commodity['collected_quantity'] / $commodity['distributed_quantity']) * 100
                                        : 0;
                                @endphp
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                @foreach($reportData['farmer_distributions'] as $farmer)
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                        @if($index === 0)
                        <td class="px-6 py-4 whitespace-nowrap" rowspan="{{ count($farmer['commodities']) + 1 }}">
                            <div class="text-sm font-medium text-gray-900 dark:text-white">
                                {{ $farmer['farmer_name'] }}
                            </div>
                            <div class="text-xs text-gray-500 dark:text-gray-400">
                            ...
                            @if($reportData['loan_type'] === 'complete-loan' && isset($farmer['return_shortfall_reason']) && $farmer['return_shortfall_reason'])
                            <div class="mt-2">
                                <span class="px-2 py-1 text-xs rounded bg-amber-100 text-amber-800 dark:bg-amber-900 dark:text-amber-300">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right">
                            @php
                                $collectionRate = $commodity['allocated'] > 0
                                    ? ($commodity['collected'] / $commodity['allocated']) * 100
                                    : 0;
                            @endphp
                        ...
                        @if($reportData['loan_type'] === 'complete-loan')
                        @if($index === 0)
                        <td class="px-6 py-4 whitespace-nowrap text-right" rowspan="{{ count($farmer['commodities']) + 1 }}">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right" rowspan="{{ count($farmer['commodities']) + 1 }}">
                            ...
                            @endphp
                            <span class="text-sm font-medium {{ $variance < 0 ? 'text-red-600 dark:text-red-400' : ($variance > 0 ? 'text-orange-600 dark:text-orange-400' : 'text-green-600 dark:text-green-400') }}">
                    ...
                    <tr class="bg-gray-50 dark:bg-gray-700 font-semibold">
                        ...
                        <td class="px-6 py-3 text-right">
                            @php
                                $totalRate = $farmer['total_allocated'] > 0
                                    ? ($farmer['total_collected'] / $farmer['total_allocated']) * 100
                                    : 0;
                            @endphp
                            <span class="text-sm text-gray-700 dark:text-gray-300">
...
@endif
        ...
        <div class="bg-red-50 dark:bg-red-900 border border-red-200 dark:border-red-700 rounded-lg p-6 text-center">
            ...
            <p class="text-red-700 dark:text-red-300">
                The tenant distribution data could not be generated. Please check the season and tenant selection and try again.
            </p>
        </div>
        @endif
    </div>
