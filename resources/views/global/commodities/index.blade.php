...
@section('content')
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
            ...
            <div class="flex justify-between items-center px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                ...
                <div class="flex space-x-2">
                    ...
                    <a href="{{ route('global.commodities.create') }}"
                    ...
                    {{-- <button onclick="openModal('marketPriceModal')"
                        class="bg-indigo-600 text-white px-4 py-2 rounded-md hover:bg-indigo-700 focus:ring-2 focus:ring-indigo-500 z-50 relative">
                        + Market Prices
                    </button> --}}
                </div>
            </div>
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            @forelse ($commodities as $item)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 text-gray-600 dark:text-gray-400">{{ $item->category->name }}</td>
                                    ...
                                    <td class="px-4 py-4">
                                        <div class="flex items-center space-x-2">
                                            <a href="{{ route('global.commodities.edit', $item->uuid) }}"
                                                class="text-blue-600 dark:text-blue-400 hover:underline text-xs">Edit</a>


                                            <form action="{{ route('global.commodities.destroy', $item->uuid) }}"
                                                method="post">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
    ...
    <div id="categoryModal" class="fixed inset-0 bg-black bg-opacity-50 hidden flex items-center justify-center">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg w-full max-w-md p-6">
            ...
            <form action="{{ route('global.commodity-categories.store') }}" method="POST">
                @csrf
                <div class="mb-4">
    ...
    <div id="marketPriceModal" class="fixed inset-0 bg-black bg-opacity-50 hidden" style="z-index: 1000;">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg w-full max-w-md p-6 absolute" style="top: 50%; left: 50%; transform: translate(-50%, -50%);">
            <h2 class="text-lg font-bold mb-4 text-gray-900 dark:text-white">Add Market Price</h2>
            <form action="{{ route('global.commodity-market-prices.store') }}" method="POST">
                @csrf
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Commodity</label>
                    <select name="global_commodity_id" required
                        class="w-full mt-1 px-3 py-2 rounded-md border bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
                ...
                <div class="mb-4">
                    ...
                    <select name="global_season_id" required
                        class="w-full mt-1 px-3 py-2 rounded-md border bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="notes" rows="3"
                        class="w-full mt-1 px-3 py-2 rounded-md border bg-white dark:bg-gray-700 text-gray-900 dark:text-white"></textarea>
                </div>
                <div class="flex justify-end space-x-2">
    ...
    <script>
        ...
        window.addEventListener('click', function(event) {
            const modal = document.getElementById('marketPriceModal');
            if (event.target === modal) {
                closeModal('marketPriceModal');
            }
        });
