...
class AgentVerificationController extends Controller
{
    public function assignedFarmers(Request $request)
    {
        ...
        $apps = Application::with(['farmer', 'farm', 'season', 'commodity_allocations', 'applicationCenter'])
        ...
        return $request->ajax()
            ? response()->json($apps)
            : view('agent.verify-collection', compact('apps', 'seasons'));
    }
    ...
    public function assignedReturns(Request $request)
    {
        ...
        $apps = Application::with(['farmer', 'season', 'applicationCenter'])
            ->whereHas('applicationCenter', fn($q) => $q->where('return_center_id', $agent->center_id))
            ->when($filter, function ($q) use ($filter) {
                $q->whereHas('farmer', fn($f) => $f->where('full_name', 'like', "%$filter%")
                    ->orWhere('registration_number', 'like', "%$filter%"));
            })
            ->when($season, fn($q) => $q->whereHas('season', fn($s) => $s->where('slug', $season)))
            ->get();

        return $request->ajax() ? response()->json($apps) : view('agent.return-list', compact('apps'));
    }
    ...
    public function storeReturn(Request $request)
    {
        $request->validate([
            ...
            'returnedCommodityPhoto' => 'nullable|image|max:4096',
            'cashPayment' => 'nullable|numeric|min:0',
            'paymentReceipt' => 'nullable|image|max:4096',
        ]);
        ...
        ReturnVerification::create($data);
